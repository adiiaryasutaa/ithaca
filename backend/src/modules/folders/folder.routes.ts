...
folderRouter.get('/', async (req: AuthRequest, res, next) => {
  try {
    const query = z
      .object({ parentId: z.string().nullable().optional(), all: z.string().optional() })
      .parse(req.query);
...
folderRouter.patch('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (body.parentId) {
      ...
      if (descendantIds.has(body.parentId))
        return res.status(400).json({
          ...
          message: 'Folder cannot be moved into itself or a child folder.',
        });
    }
    ...
    await createAuditLog(req.user!.id, 'UPDATE_FOLDER', 'folder', updated.id, {
      name: updated.name,
      updates: body,
    });
