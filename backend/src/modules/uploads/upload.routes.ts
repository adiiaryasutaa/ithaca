...
uploadRouter.post('/resumable/init', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (sizeBytes <= 0n)
      return res
        .status(400)
        .json({ code: 'UPLOAD_SIZE_REQUIRED', message: 'Valid sizeBytes required.' });
    if (sizeBytes > BigInt(env.MAX_UPLOAD_BYTES))
      return res
        .status(400)
        .json({ code: 'UPLOAD_TOO_LARGE', message: 'File exceeds max upload size.' });
...
uploadRouter.put('/resumable/chunk/:id', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (putRes.ok) {
      ...
      await createAuditLog(req.user!.id, 'UPLOAD_FILE', 'file', existingFile.id, {
