...
fileRouter.get('/preview/:token', async (req, res, next) => {
  try {
    ...
    if (!preview || preview.file.status !== 'active')
      return res
        .status(404)
        .json({ code: 'PREVIEW_NOT_FOUND', message: 'Preview token not found.' });
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        await createAuditLog(req.user!.id, 'PERMANENT_DELETE_FILE', 'file', file.id, {
          name: file.name,
        });
      } catch (error) {
        failed.push({
...
fileRouter.patch('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    await createAuditLog(req.user!.id, 'UPDATE_FILE', 'file', updated.id, {
      name: updated.name,
      updates: body,
    });
...
fileRouter.post('/:id/share', async (req: AuthRequest, res, next) => {
  try {
    ...
    return res
      ...
      .json({ url: `${env.FRONTEND_URL}/public/files/${token}`, shareId });
  } catch (error) {
    return next(error);
  }
});
...
fileRouter.post('/batch-download', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (files.length === 0)
      return res.status(404).json({ code: 'FILES_NOT_FOUND', message: 'No files found.' });
