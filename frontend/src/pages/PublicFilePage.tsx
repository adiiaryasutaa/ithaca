...
function UnsupportedPreview({ file, downloadUrl }: { file: PublicFile; downloadUrl: string }) {
  return (
    <div className="flex h-full min-h-[360px] flex-col items-center justify-center px-6 text-center text-muted-foreground">
      <div className="flex h-20 w-20 items-center justify-center rounded-full border border-slate-700 bg-slate-800 text-slate-100 shadow-2xl shadow-black/30">
        <FileArchive className="h-9 w-9" />
      </div>
      <h2 className="mt-6 text-xl font-bold text-white">Preview not available</h2>
      <p className="mt-2 max-w-md text-sm text-muted-foreground">
        {file.name} cannot be previewed in browser. Download file to open it locally.
      </p>
...
export function PublicFilePage({ embed = false }: { embed?: boolean }) {
  ...
  useEffect(() => {
    document.title = file ? `${file.name} | Ithaca` : 'Shared file | Ithaca';
    return () => {
      document.title = 'Ithaca';
    };
  }, [file]);
  ...
  if (failed) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0f1117] p-6 text-white">
        <div className="max-w-md rounded-sm border border-slate-800 bg-slate-900 p-8 text-center shadow-2xl shadow-black/30">
  ...
  if (!file) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0f1117] text-sm font-semibold text-muted-foreground">
        Loading shared file...
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#101218] text-white">
      <header className="fixed inset-x-0 top-0 z-30 flex h-16 items-center justify-between border-b border-slate-800 bg-[#17191f] px-4 shadow-lg shadow-black/20 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-sm bg-slate-800 text-slate-100">
            {fileIcon(file, kind)}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <a href={downloadUrl} download>
            <Button variant="outline" className="border-slate-800 bg-slate-800 text-white">
              <Download className="h-4 w-4" />
              Download
            </Button>
          </a>
      ...
      <section className="flex h-screen items-center justify-center px-3 pb-6 pt-20 sm:px-6">
        <div className="relative h-full w-full overflow-hidden rounded-sm border border-slate-800 bg-[#0b0d12] shadow-2xl shadow-black/40">
