...
export function UploadProvider({ children }: { children: ReactNode }) {
  ...
  async function uploadFiles(
          ...
          (filePercent) => {
            setUploadProgress((current) => {
              ...
              const overallPercent = Math.round(
                nextFiles.reduce((sum, f) => sum + f.percent, 0) / nextFiles.length,
              );
              return {
                ...current,
    ...
    window.dispatchEvent(new Event('ithaca:upload-completed'));
  }
  ...
  return (
    <UploadContext.Provider
      value={{ uploadProgress, setUploadProgress, uploadFiles, retryFailedUpload }}
    >
      {children}
    </UploadContext.Provider>
  );
